...
export default function AdminPanel() {
  ...
  const [formData, setFormData] = useState({
    name: '', price: '', category: 'Brankas Baru', height: '', length: '', width: '', weight: '', image1: '', image2: ''
  });
  ...
  const submitProduct = async (e: React.FormEvent) => {
    ...
    if(res.ok) {
      ...
      setFormData({name: '', price: '', category: 'Brankas Baru', height: '', length: '', width: '', weight: '', image1: '', image2: ''});
    } else {
          ...
          <form onSubmit={submitProduct} className="space-y-6">
            ...
            <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div><label className={labelClass}>Berat (kg)</label><input required type="number" className={inputClass} placeholder="250" onChange={e => setFormData({...formData, weight: e.target.value})} value={formData.weight} /></div>
              </div>
            </div>
